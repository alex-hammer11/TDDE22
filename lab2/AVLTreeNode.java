...
public class AVLTreeNode<T extends Comparable<T>> {
    ...
    public AVLTreeNode<T> balanceTree(AVLTreeNode<T> t) {
    	calculateHeight(this);
    	
    	if (nodeHeight(left) - nodeHeight(right) > 1) {
    		if (nodeHeight(left.left)- nodeHeight(left.right) < 0) {
    			return this.doubleRotationWithLeftChild(this);
    		} else {
    			return this.singleRotationWithLeftChild(this);
    		}
    	}
    	
    	if (nodeHeight(right) - nodeHeight(left) > 1) {
    		if (nodeHeight(right.right) - nodeHeight(right.left) < 0) {
    			return this.doubleRotationWithRightChild(this);
    		} else {
    			return this.singleRotationWithRightChild(this);
    		}
    	}
